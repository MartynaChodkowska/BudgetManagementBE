...
try
	{
		...
		else
		{
			...
			$sql = "SELECT date, amount, transactionGroup, transactionType, comment FROM transactions WHERE userId=$id AND YEAR(date) = YEAR(now()) AND transactionType='Income' ORDER BY date DESC";
			...
			$sql = "SELECT date, amount, transactionGroup, transactionType, comment FROM transactions WHERE userId=$id AND YEAR(date) = YEAR(now()) AND transactionType='Expense' ORDER BY date DESC";
			if($result = @$link->query($sql))
			{
