...
try
	{
		...
		$sql = "SELECT transactionGroup, SUM(amount) AS totalAmount FROM transactions WHERE userId= :userId AND Month(date)= :month AND Year(date)= :year AND transactionType='Income' GROUP BY transactionGroup ORDER BY totalAmount DESC";
		$incomesQuery = $db->prepare($sql);
		$incomesQuery->bindValue(':userId', $id, PDO::PARAM_STR);
		$incomesQuery->bindParam(':month', $currentMonth, PDO::PARAM_STR);
		$incomesQuery->bindParam(':year', $currentYear, PDO::PARAM_STR);
		...
		$sql = "SELECT transactionGroup, SUM(amount) as totalAmount FROM transactions WHERE userId= :userId AND Month(date)= :month AND Year(date)= :year AND transactionType='Expense' GROUP BY transactionGroup ORDER BY totalAmount DESC";
